...
import 'package:zerosoda/Screen/introPage.dart';
...
FirebaseFirestore.instance.collection('users').doc(googleUser.id).get().then((doc) async {
  print('회원 관리');
   if (!doc.exists) {
     userReference.doc(googleUser.email).set({
       'id' : googleUser.id,
...
MakingMyCalendar(press,price) async{
   await FirebaseFirestore.instance.collection('Calendar').doc('${FirebaseAuth.instance.currentUser!.displayName!}').set({
     'Calendar': press,
     'CalendarPrice' : price
     }).whenComplete(() {
